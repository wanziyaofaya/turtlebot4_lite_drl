...
class TurtleBotNavEnv(gym.Env):
    def __init__(
        ...
        # A* Subgoal (waypoint) Setup
        self.use_astar_subgoal = bool(use_astar_subgoal)
        self.astar_resolution = float(astar_resolution)
        self.astar_path = None  # list[np.ndarray([x,y])]
    ...
    def _set_current_goal_internal(self, goal_xy, using_subgoal: bool):
        """Internal: set the masqueraded goal (what RL sees) and refresh cached metrics."""
        goal_xy = np.asarray(goal_xy, dtype=np.float32)
        self.goal_position = goal_xy
        self.current_goal = goal_xy.copy()
        self.using_subgoal = bool(using_subgoal)

        # Update visualization: blue marker for current (sub)goal
        ...
        # Reset goal-history so reward remains smooth after switching goals
        ...
        self.prev_angle_to_goal = angle_to_goal_global - float(self.current_yaw)
        self.prev_angle_to_goal = (self.prev_angle_to_goal + np.pi) % (2 * np.pi) - np.pi

        # Refresh cached metrics for this step
    ...
    def _advance_astar_waypoint_if_needed(self, dist_to_current_goal: float, target: bool, collision: bool) -> bool:
        """Internal: if close to current waypoint, advance to next waypoint (or global goal)."""
        if (
            not self.use_astar_subgoal
            or self.astar_path is None
            or target
            or collision
        ):
            return False
        ...
        # Advance index (current index points to the goal RL is chasing)
        next_index = int(self.astar_path_index) + 1
        if next_index >= len(self.astar_path):
            next_goal = self.global_goal_position.copy()
        else:
            next_goal = self.astar_path[next_index].copy()

        # If we are effectively at the global goal, snap to it
        if float(np.linalg.norm(next_goal - self.global_goal_position)) < SUBGOAL_SWITCH_THRESHOLD:
            next_goal = self.global_goal_position.copy()
            using_subgoal = False
            self.astar_path_index = len(self.astar_path) - 1
        else:
            using_subgoal = True
            self.astar_path_index = next_index

        self._print_and_log(
            f"[Internal] A* waypoint switch: idx={self.astar_path_index}/{len(self.astar_path)-1} goal={next_goal}"
        )
        self._set_current_goal_internal(next_goal, using_subgoal=using_subgoal)
        return True
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self.use_astar_subgoal:
            try:
                ...
                planned = None
                self._print_and_log(f"[Internal] A* planning failed: {e}. Fallback to global goal.")

            if planned is not None and len(planned) >= 2:
                self.astar_path = [np.asarray(p, dtype=np.float32) for p in planned]

                # 你的思路：路径第二个点作为子目标（path[1]）
                idx = 1
                # 如果 path[1] 太贴近起点，则推进到更远的点（避免刚 reset 就立即切换）
                while idx < (len(self.astar_path) - 1) and float(np.linalg.norm(self.astar_path[idx] - self.current_position)) < SUBGOAL_SWITCH_THRESHOLD:
                    idx += 1
                self.astar_path_index = idx
                candidate = self.astar_path[self.astar_path_index].copy()

                # 如果候选点已经很接近全局终点，就直接用全局终点
                if float(np.linalg.norm(candidate - self.global_goal_position)) < SUBGOAL_SWITCH_THRESHOLD:
                    candidate = self.global_goal_position.copy()
                    using_subgoal = False
                else:
                    using_subgoal = True

                self._print_and_log(f"[Internal] A* path planned: {len(self.astar_path)} points | using idx={self.astar_path_index} goal={candidate}")
                self._set_current_goal_internal(candidate, using_subgoal=using_subgoal)
            else:
                self._print_and_log("[Internal] A* returned no path; using global goal.")
                self._set_current_goal_internal(self.global_goal_position.copy(), using_subgoal=False)
            ...
            if subgoal is not None:
                ...
                # 子目标太近起点 -> 直接用全局终点（避免刚开始就切换）
                    ...
                    self._print_and_log(f"[Internal] Using subgoal: {subgoal}")
                    self._set_current_goal_internal(subgoal, using_subgoal=True)
            else:
    ...
    def step(self, action):
        ...
        dist_to_current_goal = float(distance_to_goal)
        ...
        # A* waypoint：接近当前子目标就静默切到下一个 waypoint
        astar_switched = self._advance_astar_waypoint_if_needed(dist_to_current_goal, target=target, collision=collision)
        if astar_switched:
            # 切换后，刷新本 step 的度量值，确保奖励/状态一致
        ...
        # Neural subgoal（旧逻辑）：接近子目标后切到全局终点
        if (not self.use_astar_subgoal) and self.using_subgoal and (not target) and (not collision):
            if dist_to_current_goal < SUBGOAL_SWITCH_THRESHOLD:
    ...
    def _predict_subgoal(self):
        ...
        try:
            ...
            if dist_to_start < 0.3:
                self._print_and_log(f"⚡ Subgoal too close to start ({dist_to_start:.4f} < 0.3), using global goal instead.")
