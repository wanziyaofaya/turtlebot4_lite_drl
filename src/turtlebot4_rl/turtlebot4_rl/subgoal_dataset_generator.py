...
def generate_subgoal_dataset(env, model_dir, num_samples=200000, output_file='subgoal_dataset.txt', min_distance=2):
    """
    生成子目标点数据集，每条数据包括：起点、终点、子目标点、激光信息。
    确保 start、goal、lidar_data 和 A* 规划都在同一时刻、同一位置采集。
    """
    ...
    with open(dataset_path, 'a') as f:  # 使用追加模式
        ...
        for i in range(num_samples):
            ...
            # f.write(f"{start[0]:.2f},{start[1]:.2f},{goal[0]:.2f},{goal[1]:.2f},{subgoal[0]:.4f},{subgoal[1]:.4f},{lidar_str}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_samples', type=int, default=200000, help='生成样本数量')
